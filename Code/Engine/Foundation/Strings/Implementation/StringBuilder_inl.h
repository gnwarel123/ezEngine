...
inline void ezStringBuilder::ChangeCharacter(iterator& out_it, ezUInt32 uiCharacter)
{
  EZ_ASSERT_DEV(out_it.IsValid(), "The given character iterator does not point to a valid character.");
  EZ_ASSERT_DEV(out_it.GetData() >= GetData() && out_it.GetData() < GetData() + GetElementCount(),
  ...
  if (ezUnicodeUtils::IsASCII(*out_it) && ezUnicodeUtils::IsASCII(uiCharacter))
  {
    char* pPos = const_cast<char*>(out_it.GetData()); // yes, I know...
    *pPos = uiCharacter & 0xFF;
    return;
  }

  ChangeCharacterNonASCII(out_it, uiCharacter);
}
