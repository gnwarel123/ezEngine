...
ezUInt32 ezUnicodeUtils::DecodeUtf8ToUtf32(ByteIterator& out_szUtf8Iterator)
{
  return utf8::unchecked::next(out_szUtf8Iterator);
}
...
template <typename ByteIterator>
void ezUnicodeUtils::EncodeUtf32ToUtf8(ezUInt32 uiUtf32, ByteIterator& out_szUtf8Output)
{
  out_szUtf8Output = utf8::unchecked::utf32to8(&uiUtf32, &uiUtf32 + 1, out_szUtf8Output);
}
...
inline bool ezUnicodeUtils::SkipUtf8Bom(const char*& out_szUtf8)
{
  EZ_ASSERT_DEBUG(out_szUtf8 != nullptr, "This function expects non nullptr pointers");

  if (utf8::starts_with_bom(out_szUtf8, out_szUtf8 + 4))
  {
    out_szUtf8 += 3;
    return true;
  }
...
inline bool ezUnicodeUtils::SkipUtf16BomLE(const ezUInt16*& out_pUtf16)
{
  EZ_ASSERT_DEBUG(out_pUtf16 != nullptr, "This function expects non nullptr pointers");

  if (*out_pUtf16 == ezUnicodeUtils::Utf16BomLE)
  {
    ++out_pUtf16;
    return true;
  }
...
inline bool ezUnicodeUtils::SkipUtf16BomBE(const ezUInt16*& out_pUtf16)
{
  EZ_ASSERT_DEBUG(out_pUtf16 != nullptr, "This function expects non nullptr pointers");

  if (*out_pUtf16 == ezUnicodeUtils::Utf16BomBE)
  {
    ++out_pUtf16;
    return true;
  }
...
inline void ezUnicodeUtils::MoveToNextUtf8(const char*& out_szUtf8, ezUInt32 uiNumCharacters)
{
  EZ_ASSERT_DEBUG(out_szUtf8 != nullptr, "Bad programmer!");

  while (uiNumCharacters > 0)
  {
    EZ_ASSERT_DEV(*out_szUtf8 != '\0', "The given string must not point to the zero terminator.");

    do
    {
      ++out_szUtf8;
    } while (IsUtf8ContinuationByte(*out_szUtf8));

    --uiNumCharacters;
  }
}

inline void ezUnicodeUtils::MoveToNextUtf8(const char*& out_szUtf8, const char* szUtf8End, ezUInt32 uiNumCharacters)
{
  EZ_ASSERT_DEBUG(out_szUtf8 != nullptr, "Bad programmer!");

  while (uiNumCharacters > 0 && out_szUtf8 < szUtf8End)
  {
    EZ_ASSERT_DEV(*out_szUtf8 != '\0', "The given string must not point to the zero terminator.");
    ...
    {
      ++out_szUtf8;
    } while ((out_szUtf8 < szUtf8End) && IsUtf8ContinuationByte(*out_szUtf8));

    --uiNumCharacters;
  }
}

inline void ezUnicodeUtils::MoveToPriorUtf8(const char*& out_szUtf8, ezUInt32 uiNumCharacters)
{
  EZ_ASSERT_DEBUG(out_szUtf8 != nullptr, "Bad programmer!");
