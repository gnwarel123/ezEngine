...
public:
  ...
  explicit ezPointerWithFlags(PtrType* pPtr, uint8_t flags = 0) { SetPtrAndFlags(ptr, flags); }
  ...
  bool operator==(const PtrType* pPtr) const { return GetPtr() == ptr; }

  /// \brief Compares the pointer part for inequality (flags are ignored)
  bool operator!=(const PtrType* pPtr) const { return !(*this == ptr); }

  /// \brief Compares the pointer part for equality (flags are ignored)
  bool operator==(PtrType* pPtr) const { return GetPtr() == pPtr; }

  /// \brief Compares the pointer part for inequality (flags are ignored)
  bool operator!=(PtrType* pPtr) const { return !(*this == ptr); }
